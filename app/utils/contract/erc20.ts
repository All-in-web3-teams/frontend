...
const erc20 = ({ writeContractAsync }: IProps = {}) => {
  ...
  const owner = async (address: Address | null | undefined) => {
    if (!address) return null
    try {
      const result = await readContract(config, {
        abi: erc20Abi,
      ...
      console.log('owner: ', result, typeof result)
  ...
  const totalSupply = async (address: Address | null | undefined) => {
    if (!address) return null
    try {
      const result = await readContract(config, {
        abi: erc20Abi,
  ...
  const decimals = async (address: Address | null | undefined) => {
    if (!address) return null
    try {
      const result = await readContract(config, {
        abi: erc20Abi,
